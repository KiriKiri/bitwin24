...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zbwiObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zbwiObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zBWIsupply", zbwiObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZBWI()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zBWIsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zBWI denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zBWI denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zBWI denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zBWI denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zBWI denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zBWI denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zBWI denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zBWI denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zBWI denominations\n"
            "  }\n"
            "}\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZBWI_FAILED_ACCUMULATOR_INITIALIZATION);
