                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount bwiAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // BWI Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = bwiAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // BWI labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, bwiAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelBWIPercent->setText(sPercentage);
    ui->labelzBWIPercent->setText(szPercentage);
    ...
    // BWI Available
    bool showBWIAvailable = settingShowAllBalances || bwiAvailableBalance != nTotalBalance;
    ...
    // zBWI
    bool showzBWIAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzBWIUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzBWIImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzBWIAvailable);
    ui->labelzBalanceMatureText->setVisible(showzBWIAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzBWIUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzBWIUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzBWIImmature);
    ui->labelzBalanceImmatureText->setVisible(showzBWIImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelBWIPercent->setVisible(showPercentages);
    ui->labelzBWIPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BWI")
    updateDisplayUnit();
