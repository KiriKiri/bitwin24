...
bool CZBWIControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBWIControlDialog::COLUMN_DENOMINATION || column == ZBWIControlDialog::COLUMN_VERSION || column == ZBWIControlDialog::COLUMN_CONFIRMATIONS)
...
void ZBWIControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBWIControlWidgetItem* itemDenom(new CZBWIControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBWIControlWidgetItem *itemMint = new CZBWIControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zbwiTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbwiTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBWI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBWI seed used to mint this zBWI is not the same as currently hold in the wallet";
            else
