...
void PrivacyDialog::sendzBWI()
{
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BWI </b>will be added to the standard transaction fees!<br />";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BWI, ";
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        //ui->pushButtonMintzBWI->setToolTip(tr("PrivacyDialog", "Enter an amount of BWI to convert to zBWI", 0));
