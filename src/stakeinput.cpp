...
bool CZBWIStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
//!BWI Stake
bool CBWIStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBWIStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CBWIStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBWIStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CBWIStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CBWIStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CBWIStake::GetUniqueness()
{
    //The unique identifier for a BWI stake is the outpoint
...
CBlockIndex* CBWIStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
