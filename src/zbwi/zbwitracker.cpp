...
CoinWitnessData* CzBWITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBWITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
void CzBWITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBWIWallet* zBWIWallet)
{
    bool iszBWIWalletInitialized = (NULL != zBWIWallet);
    CMintMeta meta;
    ...
    if (! iszBWIWalletInitialized)
        zBWIWallet = new CzBWIWallet(strWalletFile);
    meta.isSeedCorrect = zBWIWallet->CheckSeed(dMint);
    if (! iszBWIWalletInitialized)
        delete zBWIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
std::set<CMintMeta> CzBWITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBWIWallet* zBWIWallet = new CzBWIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBWIWallet);
        }
        delete zBWIWallet;
