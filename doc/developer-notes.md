...
- [Development guidelines](#development-guidelines)
    - [General Bitwin24 Core](#general-bitwin24-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitwin24 Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Bitwin24 Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bitwin24-project/bitwin24/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitwin24
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitwin24 --log_level=test_suite
$ valgrind -v --leak-check=full src/bitwin24d -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bitwin24.coverage/index.html`.
```
...
- BitcoinMiner : Generates BWIs (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against Bitwin24 Core so that they can be integrated
...
file descriptors it will cause problems with Bitwin24's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitwin24-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitwin24-cli` and the GUI debug console use this table to determine how to
